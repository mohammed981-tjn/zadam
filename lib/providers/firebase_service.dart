...
class FirebaseService {
  ...
  Stream<List<Restaurant>> streamRestaurants() =>
      _restaurants.orderBy('name').snapshots().map((s) => s.docs
          .map((d) => Restaurant.fromMap(d.data(), d.id))
          .toList());

  Future<void> addRestaurant(Restaurant r) =>
      _restaurants.doc(r.id).set(r.toMap());

  Future<void> updateRestaurant(Restaurant r) =>
  ...
  //  MENU CATEGORIES
  // ══════════════════════════════════════════════════════════
  Stream<List<MenuCategory>> streamCategories(String rId) =>
      _categories(rId)
          .orderBy('sortOrder')
          .snapshots()
          .map((s) => s.docs
              .map((d) => MenuCategory.fromMap(d.data(), d.id))
              .toList());

  Future<void> addCategory(MenuCategory cat) =>
      _categories(cat.restaurantId).doc(cat.id).set(cat.toMap());

  Future<void> updateCategory(MenuCategory cat) =>
  ...
  //  MENU ITEMS
  // ══════════════════════════════════════════════════════════
  Stream<List<MenuItem>> streamMenuItems(String rId) =>
      _items(rId).snapshots().map((s) =>
          s.docs.map((d) => MenuItem.fromMap(d.data(), d.id)).toList());
  ...
  Stream<List<Driver>> streamDrivers() =>
      _drivers.snapshots().map((s) =>
          s.docs.map((d) => Driver.fromMap(d.data(), d.id)).toList());
  ...
  Future<void> addDriver(Driver d) =>
      _drivers.doc(d.id).set(d.toMap());

  Future<void> updateDriver(Driver d) =>
  ...
  Future<void> updateDriverRating(
    ...
    final driver = Driver.fromMap(doc.data()!, doc.id);
  ...
  Future<String> placeOrder(Order order) async {
    ...
    // ✅ تحديث المخزون لكل صنف يتتبع المخزون
    // (يتم بشكل منفصل بعد التأكيد من المدير)
  ...
  Stream<List<Order>> streamDriverOrders(String driverId) =>
      _orders
  ...
  Future<void> markOrderDelivered(String orderId, String driverId) async {
    ...
    if (orderDoc.exists && orderDoc.data() != null) {
      final order = Order.fromMap(orderDoc.data()!, orderDoc.id);
  ...
  Stream<List<Complaint>> streamComplaints() =>
      _complaints
          ...
          .snapshots()
          .map((s) => s.docs
              .map((d) => Complaint.fromMap(d.data(), d.id))
              .toList());

  Stream<List<Complaint>> streamCustomerComplaints(String customerId) =>
      _complaints
          .where('customerId', isEqualTo: customerId)
          .orderBy('createdAt', descending: true)
          .snapshots()
